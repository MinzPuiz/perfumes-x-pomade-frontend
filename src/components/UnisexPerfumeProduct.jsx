...
const UnisexPerfumeProduct = ({ product }) => {
  ...
  const modalContent = (
    <div className="modal-overlay">
      <div className="modal-content max-w-xl w-full">
        ...
        <button
          onClick={toggleModal}
          className="mt-6 bg-yellow-600 text-white px-4 py-2 rounded-full hover:bg-yellow-700 transition"
        >
          Đóng
        </button>
      </div>
    ...
    <>
      <div className="relative group bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition duration-300">
        ...
        <div className="p-4">
          ...
          <p className="price text-[#c97b3d]">{Number(product.price).toLocaleString()}đ</p>
        </div>
      </div>
...
export default UnisexPerfumeProduct;
